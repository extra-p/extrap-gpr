...
def main():
    ...
    parser.add_argument('--filter', type=int, required=False, help='Set the filter that was used for the analysis.')
    ...
    if args.filter:
        fig.suptitle("Evaluation results RELEARN ("+str(nr_func_modeled)+" kernels > "+str(args.filter)+"% runtime)")
    else:
        fig.suptitle("Evaluation results RELEARN ("+str(nr_func_modeled)+" kernels)")
    
    # plot the accuracy of bucket 5
    ls=["-",'dotted','--',':','-']
    lw = [1,2,2,5,2]
    #ax1.set_xscale("symlog")
    ax1.set_xlim(args.min, 105)
    style_counter = 0
    zorders=[5,4,3,2,1]
    ...
    ax2.set_xticks(np.arange(x_min_tick, 110, 10))
    ...
    ax3.set_xticks(np.arange(x_min_tick, 110, 10))
    ...
    ax4.set_xticks(np.arange(x_min_tick, 110, 10))
    ...
    ax5.set_xticks(np.arange(x_min_tick, 110, 10))
    ax5.set_ylim(0,100)
    ax5.set_xlim(args.min,100)
    ...
    ax6.set_xticks(np.arange(x_min_tick, 110, 10))
    ax6.set_yticks(np.arange(0, ((25-1)*reps)+5, 5))
    ax6.set_xlim(args.min,105)
