...
def plot_model_accuracy(percentage_bucket_counter_full, percentage_bucket_counter_generic, percentage_bucket_counter_gpr, percentage_bucket_counter_hybrid):
    X = ['+-5%','+-10%','+-15%','+-20%']
...
def plot_costs(used_costs, base_budget):
...
def plot_measurement_point_number(add_points, min_points):
    langs = ["full", "generic", "gpr", "hybrid\n(generic+gpr)"]
    fig, ax = plt.subplots()
    bottom = np.zeros(4)
    bars = []
